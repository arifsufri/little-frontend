...
type BossRegisterForm = z.infer<typeof BossRegisterSchema>;
...
export default function BossPortalPage() {
  ...
  } = useForm<BossRegisterForm>({ resolver: zodResolver(BossRegisterSchema) });
  ...
  const onSubmit = async (data: BossRegisterForm) => {
    setErrorMsg(null);
    try {
      const response = await apiPost<ApiResponse>('/auth/register-boss', {
        name: data.name,
        email: data.email,
      ...
      if (response.success && response.data) {
        ...
        throw new Error(response.message || 'Failed to create Boss account');
      }
    } catch (e: any) {
      const msg = e?.response?.data?.message || e?.response?.data?.error || e?.message || 'Boss registration failed';
      setErrorMsg(msg);
    }
  };
          ...
          <Card sx={{
            ...
            <CardContent sx={{ p: { xs: 3, sm: 4 } }}>
              ...
              <Typography variant="body2" color="text.secondary" textAlign="center" mt={0.5}>
                Exclusive registration for Boss accounts
              </Typography>
